...
#include "pending_map_task_partition.h"
...
private:
	...
	void send_tasks_partition(std::string worker_id, PendingMapTaskPartition *current_partition_to_send);
	...
	void save_logs();
	void update_nodes_state(PendingMapTaskPartition *map_task, std::string worker_id);
	...
	std::list<PendingMapTaskPartition*> pending_maps;
