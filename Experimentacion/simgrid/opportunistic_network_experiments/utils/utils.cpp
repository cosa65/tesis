...
namespace Utils {
	template <class T>
	std::list<std::list<T>*> generate_list_with_empty_lists(int size) {
		std::list<std::list<T>*> *result = new std::list<std::list<T>*>(size);
		std::generate(result -> begin(), result -> end(), [](){ return new std::list<T>(); });
	...
	template <class T>
	std::list<std::list<T>*> separate_in_partitions(std::list<T> list, int amount_of_partitions) {
		...
		std::list<std::list<T>*> partitions_list = generate_list_with_empty_lists<T>(amount_of_partitions);
		...
		for (; partition_size <= std::distance(list_it, list.end()); std::advance(list_it, partition_size)) {
			std::list<T> *partition = new std::list<T>();
	...
	template <class T>
	void join_lists(std::list<std::list<T>*> list1, std::list<std::list<T>*> list2) {
