...
namespace Utils {
	std::list<std::list<int>*> generate_list_with_empty_lists(int size) {
		std::list<std::list<int>*> *result = new std::list<std::list<int>*>(size);
		std::generate(result -> begin(), result -> end(), [](){ return new std::list<int>(); });
	...
	std::list<std::list<int>*> separate_in_partitions(std::list<int> list, int amount_of_partitions) {
		...
		std::list<std::list<int>*> partitions_list = generate_list_with_empty_lists(amount_of_partitions);
		...
		for (; partition_size <= std::distance(list_it, list.end()); std::advance(list_it, partition_size)) {
			std::list<int> *partition = new std::list<int>();
